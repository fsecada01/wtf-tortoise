...
async def list_all(request):
    ...
    backward_fk_fields = [m["name"]
                          for m in model_describe.get("backward_fk_fields")]
    field_name_list = [
        i for i in Book._meta.fields_map if i not in backward_fk_fields]
    if fk_fields:
        results = (
            await Book.all()
            .prefetch_related(*fk_fields)
        )
    else:
        results = (
            await Book.all()
            .values(*field_name_list)
        )
...
routes = [
    Route("/", endpoint=list_all,
          methods=["GET"], name="list_all"),
    Route("/create", endpoint=create,
          methods=["GET", "POST"], name="create"),
    Route("/{id:int}/edit", endpoint=edit,
          methods=["GET", "POST"], name="edit"),
    Route("/{id:int}/delete", endpoint=delete,
